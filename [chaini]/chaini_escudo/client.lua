...
local pistol = GetHashKey("WEAPON_PISTOL50")

RegisterNetEvent('chaini_escudo:escudo')
AddEventHandler('chaini_escudo:escudo',function()
...
function EnableShield()
    ...
    shield = CreateObject(GetHashKey(prop), pedPos.x, pedPos.y, pedPos.z, 1, 1, 1)
    shieldEntity = shield
...
function DisableShield()
    local ped = GetPlayerPed(-1)
    Citizen.Trace(tostring(shieldEntity))
    DeleteObject(shieldEntity)
