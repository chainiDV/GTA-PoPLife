...
ESX.RegisterUsableItem('bag', function(source)
    ...
    TriggerClientEvent('esx-kr-bag:CheckBag', source)
end)

RegisterServerEvent('esx-kr-bag:putBag')
AddEventHandler('esx-kr-bag:putBag', function(HasBag)
    local xPlayer = ESX.GetPlayerFromId(source)
    ...
    if not HasBag then
        ...
        TriggerClientEvent('esx:showNotification', source, '~r~Ya tienes una bolsa.')
    end
end)
...
AddEventHandler('esx-kr-bag:PutItem', function(id, item, label, count, type)
    ...
    elseif type == 'item' and count < Config.MaxItemCount then
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_bag_inventory WHERE id = @id ',{["@id"] = id}, function(result)
            ...
            TriggerClientEvent('esx:showNotification', src, '~r~Solo puedes tener ' .. Config.MaxItemCount .. ' objetos diferentes en tu mochila')
        end
    end)
end)
...
AddEventHandler('esx-kr-bag:PickUpBag', function(id)
	...
	TriggerClientEvent('esx:showNotification', src, 'Presiona ~b~[5]~w~ para acceder a tu mochila.')
        TriggerClientEvent('esx-kr-bag:ReSync', xPlayers[i], id)
     end
end)
