...
function StartTheoryTest()
  ...
  TriggerServerEvent('esx_dmvschool:pagoPop', Config.Prices['dmv'])
...
function StopTheoryTest(success)
  ...
  if success then
    ...
    ESX.ShowNotification("Has pasado el test")
  else
    ESX.ShowNotification("Has fallado el test")
  end
...
function OpenDMVSchoolMenu()
  ...
  if not ownedLicenses['dmv'] then
    table.insert(elements, {label = "Test teórico " .. Config.Prices['dmv'], value = 'theory_test'})
  end

  if ownedLicenses['dmv'] then

    if not ownedLicenses['drive'] then
      table.insert(elements, {label = "Test de conducir " .. Config.Prices['drive'], value = 'drive_test', type = 'drive'})
    end

    if not ownedLicenses['drive_bike'] then
      table.insert(elements, {label = "Test de moto " .. Config.Prices['drive_bike'], value = 'drive_test', type = 'drive_bike'})
    end

    if not ownedLicenses['drive_truck'] then
      table.insert(elements, {label = "Test de camion " .. Config.Prices['drive_truck'], value = 'drive_test', type = 'drive_truck'})
    end
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      CurrentActionMsg  = 'Presiona ~INPUT_CONTEXT~ para abrir el menu'
      CurrentActionData = {}
...
Citizen.CreateThread(function()
  ...
  AddTextComponentString("Auto-Escuela")
  ...
  while true do
    ...
    if CurrentTest == 'drive' then
      ...
      if Config.CheckPoints[nextCheckPoint] == nil then
        ...
        ESX.ShowNotification("El test ha sido completado")
    ...
    if CurrentTest == 'drive' then
      ...
      if IsPedInAnyVehicle(playerPed,  false) then
        ...
        for k,v in pairs(Config.SpeedLimits) do
          if CurrentZoneType == k and speed > v then
            ...
            if not IsAboveSpeedLimit then
              ...
              ESX.ShowNotification("Has superado el limite de " .. v .. 'km/h')
              ESX.ShowNotification("Errores: " .. DriveErrors .. '~s~/' .. Config.MaxErrors)
        ...
        if health < LastVehicleHealth then
          ...
          ESX.ShowNotification("Has dañado el vehículo")
          ESX.ShowNotification("Errores : " .. DriveErrors .. '~s~/' .. Config.MaxErrors)
