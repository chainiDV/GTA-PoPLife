...
RegisterServerEvent('esx_property:comprarPropiedadPop')
AddEventHandler('esx_property:comprarPropiedadPop', function(propertyName)
...
RegisterServerEvent('esx_property:getitemPop')
AddEventHandler('esx_property:getitemPop', function(owner, type, item, count)
	...
	if type == 'item_standard' then
		...
		TriggerEvent('esx_addoninventory:getInventory', 'property', xPlayerOwner.identifier, function(inventory)
			...
			if roomItemCount >= count and count > 0 then
	...
	if type == 'item_account' then
		...
		TriggerEvent('esx_addonaccount:getCuentaPop', 'property_' .. item, xPlayerOwner.identifier, function(account)

			local roomAccountMoney = account.money

			if roomAccountMoney >= count and count > 0 then
...
RegisterServerEvent('esx_property:ponerItemPop')
AddEventHandler('esx_property:ponerItemPop', function(owner, type, item, count)
...
ESX.RegisterServerCallback('esx_property:getPropertyInventory', function(source, cb, owner)
	...
	TriggerEvent('esx_addonaccount:getCuentaPop', 'property_black_money', xPlayer.identifier, function(account)
			...
			MySQL.Async.fetchAll(
				...
				function(result)
					...
					for i=1, #result, 1 do
						...
						TriggerEvent('esx_addonaccount:getCompartidaCuentaPop', 'society_realestateagent', function(account)
