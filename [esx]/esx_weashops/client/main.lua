...
function OpenShopMenu(zone)
	...
	for i=1, #Config.Zones[zone].Items, 1 do
		...
		if donationsfinish == true then
			table.insert(elements, {
				label     = item.label .. ' - <span style="color:green;">$' .. item.price * 0.5 .. ' </span>',
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)				
			TriggerServerEvent('esx_weashop:comprarItemPop', data.current.value, data.current.price, zone)
		end,
		function(data, menu)
...
RegisterNetEvent('pop_university:setWeaponLicense')
AddEventHandler('pop_university:setWeaponLicense', function(zone)
	weaponLicense = true
end)
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlJustReleased(0, 38) then        
        ...
        if CurrentAction == 'shop_menu' then
        	if weaponLicense == true then
        		OpenShopMenu(CurrentActionData.zone)
        	else
        		TriggerEvent('exp:NotificateError','A ver retrasao, a ver si te queda claro. Vete al puto cole a aprender como cojones usar un trabuco.')
        	end
