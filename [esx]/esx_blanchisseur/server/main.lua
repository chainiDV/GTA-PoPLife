	...
	local source = source
		...
		if PlayersWashing[source] == true then
			...
			if blackMoney.money < Config.Slice then
				...
				xPlayer.anadirMoneypop(washedMoney)
...
RegisterServerEvent('esx_blanchisseur:limpioDineroPop')
AddEventHandler('esx_blanchisseur:limpioDineroPop', function(amount)
	local xPlayer 		= ESX.GetPlayerFromId(source)
...
RegisterServerEvent('esx_blanchisseur:startLimpiadoPop')
AddEventHandler('esx_blanchisseur:startLimpiadoPop', function(percent)
