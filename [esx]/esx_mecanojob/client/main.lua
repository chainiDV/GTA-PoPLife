    ...
    function(data, menu)
      if data.current.value == 'vehicle_list' then
        ...
        if Config.EnableSocietyOwnedVehicles then
            ...
            local elements = {
              {label = "Camion", value = 'flatbed'},
              {label = "Camion", value = 'Grua'}
            }
            ...
            if amount == nil then
              ...
              if amount >= 0 and amount <= 1500 then
                ...
                if closestPlayer == -1 or closestDistance > 3.0 then
                  ...
                  TriggerServerEvent('esx_billing:enviarBillPop', GetPlayerServerId(closestPlayer), 'society_mecano', _U('mechanic'), amount)
                end
              else
                ESX.ShowNotification("No puedes ingresar esa cantidad")
              end
            end
          end,
        function(data, menu)
          ...
          if CurrentlyTowedVehicle == nil then
            ...
            if NPCOnJob then
              ...
              if CurrentlyTowedVehicle == NPCTargetTowable then
                ...
                TriggerServerEvent('esx_mecanojob:NPCMisionCompletadaPop')
                StopNPCJob()
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if PlayerData.job ~= nil and PlayerData.job.name == 'mecano' and false then
			local coords = GetEntityCoords(GetPlayerPed(-1))
			DrawMarker(1, deleteSpawn.x, deleteSpawn.y, deleteSpawn.z - 1, 0, 0, 0, 0, 0, 0, 4.5001, 4.5001, 0.4001,0,255,0, 200, 0, 0, 0, 0)
			if GetDistanceBetweenCoords(coords, deleteSpawn.x, deleteSpawn.y, deleteSpawn.z, true) < 5.0 then
				DrawText3D(deleteSpawn.x, deleteSpawn.y, deleteSpawn.z, "Deja el vehículo solo, y pulsa [E] para abrir el menú", 0,255,0)
...
function OpenVehicleSeller()
	...
	ESX.UI.Menu.Open(
		...
		{
			title  = "Desguazar vehículo",
			align    = 'bottom-right',
			elements = elements
		},
		function(data, menu)
			local coords = {x = deleteSpawn.x, y = deleteSpawn.y, z = deleteSpawn.z}
        ...
        if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then
          ...
          ESX.Game.DeleteVehicle(vehicle)
        TriggerServerEvent('exp:addExperience',exp)
        TriggerServerEvent('esx_mecano:paythem',money)
        ESX.UI.Menu.CloseAll()
        end
				

			else
				ESX.ShowNotification("No hay ningún vehículo cerca")
			end
