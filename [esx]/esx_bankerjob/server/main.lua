...
AddEventHandler('esx_bankerjob:customerDeposit', function (target, amount)
  ...
  TriggerEvent('esx_addonaccount:getCompartidaCuentaPop', 'society_banker', function (account)
    if amount > 0 and account.money >= amount then
      account.retirarDineropop(amount)

      TriggerEvent('esx_addonaccount:getCuentaPop', 'bank_savings', xPlayer.identifier, function (account)
        account.anadirMoneypop(amount)
      end)
    else
...
function CalculateBankSavings (d, h, m)
  ...
  MySQL.Async.fetchAll(
    ...
    function (result)
        ...
        if foundPlayer then
          TriggerEvent('esx_addonaccount:getCuentaPop', 'bank_savings', xPlayer.identifier, function (account)
            ...
            table.insert(asyncTasks, function(cb)
              account.anadirMoneypop(interests)
            end)
          end)
        else
      ...
      TriggerEvent('esx_addonaccount:getCompartidaCuentaPop', 'society_banker', function (account)
        account.anadirMoneypop(bankInterests)
      end)
