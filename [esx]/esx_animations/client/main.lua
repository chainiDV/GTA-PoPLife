...
RegisterNetEvent('esx_animations:handCuff')
AddEventHandler('esx_animations:handCuff',function()
...
function OpenAnimationsMenu()
	if isHandCuffed == false then
		...
		table.insert(elements, {label = "Cancelar animación", value = "cancel_animation"})
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				if not(data.current.value == 'cancel_animation') then
