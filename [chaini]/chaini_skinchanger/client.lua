...
RegisterNetEvent('chaini_skinchanger:cambioRopa')
AddEventHandler('chaini_skinchanger:cambioRopa',function()
	startClothes()
	ClothShop()
end)
...
function ClothShop()
	...
	drawTxt(0.177, 0.066, 0.25, 0.03, 0.40,"Ropa",255,255,255,255) -- header
	...
	if cmenu.show == 1 then
		...
		drawTxt(0.177, 0.128, 0.25, 0.03, 0.40,"Cambiar ropa",255,255,255,255)
		drawTxt(0.177, 0.165, 0.25, 0.03, 0.40,"Accesorios",255,255,255,255) -- row_2 (+0.037)
	--	drawTxt(0.177, 0.202, 0.25, 0.03, 0.40,"Player Model",255,255,255,255) -- row_2 (+0.037)
		drawTxt(0.177, 0.239, 0.25, 0.03, 0.40,"Salir",255,255,255,255) -- row_2 (+0.037)
	elseif cmenu.show == 2 then
		...
		drawTxt(0.177, 0.128, 0.25, 0.03, 0.40,"~b~Cambiar ropa",255,255,255,255)
		drawTxt(0.177, 0.165, 0.25, 0.03, 0.40,"Accesorios",255,255,255,255) -- row_2 (+0.037)
		--drawTxt(0.177, 0.202, 0.25, 0.03, 0.40,"Player Model",255,255,255,255) -- row_2 (+0.037)
		drawTxt(0.177, 0.239, 0.25, 0.03, 0.40,"Salir",255,255,255,255) -- row_2 (+0.037)
		---
		DrawRect(0.328,0.051,0.18,0.049,0,0,0,150) -- title
		drawTxt(0.382,0.048,0.175,0.035, 0.40,"Cambiar ropa",255,255,255,255)
		...
		if GetNumberOfPedDrawableVariations(GetPlayerPed(-1), cmenu.field-1) ~= 0 and GetNumberOfPedDrawableVariations(GetPlayerPed(-1), cmenu.field-1) ~= false then
			...
			drawTxt(0.328,0.130,0.175,0.035, 0.40,"SLOT VACÍO",255,255,255,255)
			drawTxt(0.328,0.167,0.175,0.035, 0.40,"SLOT VACÍO",255,255,255,255)
			drawTxt(0.328,0.204,0.175,0.035, 0.40,"SLOT VACÍO",255,255,255,255)
			if IsControlJustPressed(1, 189) or IsDisabledControlJustPressed(1, 189) then -- left
				PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
				if cmenu.field > 1 then 
					cmenu.field = cmenu.field-1
	...
	elseif cmenu.show == 3 then
		...
		DrawRect(0.12,0.246,0.22,0.035,0,0,0,150)
		drawTxt(0.177, 0.128, 0.25, 0.03, 0.40,"Cambiar ropa",255,255,255,255)
		drawTxt(0.177, 0.165, 0.25, 0.03, 0.40,"~b~Accesorios",255,255,255,255) -- row_2 (+0.037)
		--drawTxt(0.177, 0.202, 0.25, 0.03, 0.40,"Player Model",255,255,255,255) -- row_2 (+0.037)
		drawTxt(0.177, 0.239, 0.25, 0.03, 0.40,"Salir",255,255,255,255) -- row_2 (+0.037)
		---
		DrawRect(0.328,0.051,0.18,0.049,0,0,0,150) -- title
		drawTxt(0.382,0.048,0.175,0.035, 0.40,"Accesorios",255,255,255,255)
		DrawRect(0.328,0.024,0.175,0.005,58,95,205,150)
		if cmenu.row == 1 then DrawRect(0.328,0.096,0.18,0.035,76,88,102,150) else DrawRect(0.328,0.096,0.18,0.035,0,0,0,150) end
		if cmenu.row == 2 then DrawRect(0.328,0.133,0.18,0.035,76,88,102,150) else DrawRect(0.328,0.133,0.18,0.035,0,0,0,150) end
		if cmenu.row == 3 then DrawRect(0.328,0.170,0.18,0.035,76,88,102,150) else DrawRect(0.328,0.170,0.18,0.035,0,0,0,150) end
...
Citizen.CreateThread(function()
	while true do
		Wait(0)
		if IsControlJustPressed(1, 214) or IsDisabledControlJustPressed(1, 214) and xPlayer.job.name == 'police' then -- delete
			if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),-451.8,6005.6,31.84) < 25.0 then
				if cmenu.show == 0 then
					PlaySound(-1, "FocusIn", "HintCamSounds", 0, 0, 1)
					startClothes()
				end
			else
				TriggerEvent('chatMessage',"Ropero",{255,0,0}, "Debes estar en comisaría")
			end
		end
		if cmenu.show == 1 then
			ClothShop()
