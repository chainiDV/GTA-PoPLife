		...
		function(result)
			...
			for i=1, #result, 1 do
				...
				table.insert(shopItems[result[i].name], {
					...
					label = GetWeaponLabel(result[i].item)
				})
				print(GetWeaponLabel(result[i].item))
			end
...
RegisterServerEvent('esx_weashop:comprarItemPop')
AddEventHandler('esx_weashop:comprarItemPop', function(itemName, price, zone)
...
AddEventHandler('pop_university:setWeaponLicense', function()
	print("Me han llamado")
		TriggerClientEvent('pop_university:setWeaponLicense', source)
end)
