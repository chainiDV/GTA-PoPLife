...
RegisterServerEvent('esx:enaddInventoryItem')
AddEventHandler('esx:enaddInventoryItem', function(source, item, count)
  ...
  if currentInventoryWeight > Config.Limit then
    ...
    TriggerClientEvent('esx:showNotification', source_, 'Demasiado peso. Dejaste caer' .. item.label .. ' x' .. qty)
    xPlayer.removeInventoryItem(item.name, qty)
  end
    TriggerClientEvent('esx_advanced_inventory:updateWeight',source,currentInventoryWeight)
end)

RegisterServerEvent('esx:enremoveInventoryItem')
AddEventHandler('esx:enremoveInventoryItem', function(source, item, count)
...
RegisterServerEvent('esx_advanced_inventory:loaded')
AddEventHandler('esx_advanced_inventory:loaded', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local currentInventoryWeight = getInventoryWeight(xPlayer)
    TriggerClientEvent('esx_advanced_inventory:updateWeight',source,currentInventoryWeight)
end)
