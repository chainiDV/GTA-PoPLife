...
function OpenStealMenu(target, target_id)
    ...
    Citizen.CreateThread(function()
        while true do
            local dis1 = GetEntityCoords(GetPlayerPed(-1))
            local dis2 = GetEntityCoords(GetPlayerPed(target))
            Citizen.Trace(tostring(GetDistanceBetweenCoords(dis1.x,dis1.y,dis1.z,dis2.x,dis2.y,dis2.z)))
            if GetDistanceBetweenCoords(dis1.x,dis1.y,dis1.z,dis2.x,dis2.y,dis2.z) > 2 then
                ESX.UI.Menu.CloseAll()
                break
            end
            Citizen.Wait(0)
                    ...
                    function(data2, menu2)
                        ...
                        if data2.current.action == 'steal' then
                            ...
                            if itemType == 'item_standard' then
                                ESX.UI.Menu.Open(
                                    ...
                                    function(data3, menu3)
                                        local quantity = tonumber(data3.value)
                                            ...
                                            TriggerServerEvent('esx_thief:robarPlayerItemPop', GetPlayerServerId(target), itemType, itemName, quantity)
                                ...
                                TriggerServerEvent('esx_thief:robarPlayerItemPop', GetPlayerServerId(target), itemType, itemName, amount)
                                OpenStealMenu(target)
                            end
...
function getAreaPlayers(rango)
	local players = ESX.Game.GetPlayers()
	local playerCercanos = 0
	for i,v in pairs(players) do
		local target = GetPlayerPed(players[i])
		local targetCoords = GetEntityCoords(target)
		local coords = GetEntityCoords(GetPlayerPed(-1))
		if GetDistanceBetweenCoords(targetCoords.x, targetCoords.y, targetCoords.z, coords.x, coords.y, coords.z, true) <= rango then
			playerCercanos = playerCercanos + 1
		end
	end
	return playerCercanos
end
		...
		if IsControlJustPressed(1, Keys['G']) and IsPedArmed(ped, 7) and not IsEntityDead(ped) and not IsPedInAnyVehicle(ped, true) then
            ...
            if target ~= -1 and distance ~= -1 and distance <= 2.0 then
                ...
                IsAbleToSteal(target_id, function(err)
                    if(not err)then
                    	if getAreaPlayers(7.0) > 2 then
                    		ESX.ShowNotification("Quedate a solas con la morralla para robar")
                    	else
