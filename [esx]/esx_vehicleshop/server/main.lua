...
RegisterServerEvent('pop_donator:getJob')
AddEventHandler('pop_donator:getJob', function ()
  local xPlayer = ESX.GetPlayerFromId(source)
  local source = source
	if xPlayer then
		local identifier = xPlayer.identifier

		MySQL.Async.fetchAll('SELECT * FROM users WHERE identifier = @player', {["@player"] = xPlayer.identifier}, function(players)
			if players[1] then
				TriggerClientEvent('pop_donator:setJob',source,players[1].isDonator)
			end
		end)
    end
end)
...
AddEventHandler('esx_vehicleshop:setVehicleOwned', function (vehicleProps, tengo)
	...
	MySQL.Async.execute('INSERT INTO vehicle_sold (client, model, plate, soldby, date) VALUES (@client, @model, @plate, @soldby, @date)', {
		['@client'] = xPlayer.identifier,
		['@model'] = vehicleProps.model,
		['@plate'] = vehicleProps.plate,
		['@soldby'] = "Concesionario",
		['@date'] = dateNow
	})
end)
