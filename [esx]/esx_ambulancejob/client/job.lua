	...
	while true do
		...
		for hospitalNum,hospital in pairs(Config.Hospitals) do
			for k,v in ipairs(hospital.Vehicles) do
				...
				DrawMarker(1,v.Spawner.x,v.Spawner.y,v.Spawner.z - 1, 0, 0, 0, 0, 0, 0, 1.5001, 1.5001, 0.6001,255,255,51, 200, 0, 0, 0, 0)
				...
				if distance < v.Marker.x then
					if IsControlJustReleased(0, Keys['E']) then
						...
						OpenVehicleSpawnerMenu(currentHospital, currentPartNum)
					end
				end
			end
			for k,v in ipairs(hospital.Helicopters) do
				...
				if distance < v.Marker.x then
					if IsControlJustReleased(0, Keys['E']) then
						isInMarker, currentHospital, currentPart, currentPartNum = true, hospitalNum, 'Helicopters', k
						OpenHelicopterSpawnerMenu(currentHospital, currentPartNum)
					end
				end
			end
		end
	...
	ESX.UI.Menu.Open(
	  'default', GetCurrentResourceName(), 'cloakroom',
	  {
		...
		elements = {
		  ...
		  {label = 'Ropa de mujer' , value = 's_m_m_prisguard_01'},
		  {label = 'Limpiar Accesorios' , value = 'skin', id = 10},
		  {label = 'Uniforme Bombero' , value = 'skin', id = 1},
		  {label = 'Uniforme Enfermero' , value = 'skin', id = 3},
		  {label = 'Uniforme Técnico' , value = 'skin', id = 4},
		  {label = 'Uniforme Médico' , value = 'skin', id = 5},
		  {label = 'Uniforme Doctor' , value = 'skin', id = 6},
		  {label = 'Uniforme Jefatura' , value = 'skin', id = 7},
		  {label = 'UMIR' , value = 'skin', id = 8},
		  {label = 'Forense' , value = 'skin', id = 9},
		  {label = 'Quitar chaleco' , value = 'chaleco_out'},
		  {label = 'Añadir estetoscopio' , value = 'estetoscopio'},
		  {label = 'Variación Pantalones' , value = 'cambio_pantalones'},
		},
	  },
	  function(data, menu)
		...
		if data.current.value == 's_m_m_prisguard_01' then
		...
		if data.current.value == 'chaleco_out' then
		...
		if data.current.value == 'cambio_pantalones' then
		...
		if data.current.value == 'estetoscopio' then
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	local elements = {
		...
		{label = "Recuperar vehículo", action = 'recovery_vehicle'}
	}
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if ESX.PlayerData.job.grade_name ~= "bombero-primera" then
				authorizedVehicles = Config.AuthorizedVehicles["bomberoprimera"]
			elseif ESX.PlayerData.job.grade_name ~= "bombero-segunda" then
				authorizedVehicles = Config.AuthorizedVehicles["bomberosegunda"]
			elseif ESX.PlayerData.job.grade_name ~= "vir-tercero" then
				authorizedVehicles = Config.AuthorizedVehicles["virtercero"]
			else
		...
		elseif data.current.action == 'recovery_vehicle' then
			TriggerServerEvent('esx_ambulancejob:recovery')
			ESX.UI.Menu.CloseAll()
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			...
			local attempts = 0
			TriggerServerEvent('llaves:removearrayllaves',GetVehicleNumberPlateText(vehicleId.vehicle))
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			local shopCoords = vector3(-359.4, 6120.68, 37.8)
			local shopElements = {}
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_garage', {
						...
						if data2.current.stored then
							...
							if true then
								...
								ESX.Game.SpawnVehicle(data2.current.model, vector3(-359.4, 6120.68, 37.8), 142.7, function(vehicle)
									ESX.Game.SetVehicleProperties(vehicle, data2.current.vehicleProps)
