...
function sendNotifications(rol,msg,position,anon)
	local xPlayers = ESX.GetPlayers()
	print("p2")
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		if xPlayer.job.name == rol then
			--print("p3 -> "..xPlayers[i])
			--TriggerClientEvent('exp:Notificate',xPlayers[i],'<span style="color:red">['.. owner ..']</span> '..msg)
			TriggerClientEvent('esx_phone:onMessage', xPlayers[i], xPlayer.get('phoneNumber'), msg, position, anon, rol, false)
		end
	end
end
...
AddEventHandler('esx_phone:send', function(phoneNumber, message, anon, position)
	...
	if phoneNumber == 'police' or phoneNumber == 'ambulance' or phoneNumber == 'mecano' or phoneNumber == 'taxi' then
		sendNotifications(phoneNumber,message,position,anon)
	end
