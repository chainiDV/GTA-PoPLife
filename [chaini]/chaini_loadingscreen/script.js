...
VK.info.change = function () {
    setTimeout(function () {
        let newPosition = Math.round(getRandomArbitrary(0, VK.info.list.length - 1));
        while (newPosition == VK.info.actual) {
            newPosition = Math.round(getRandomArbitrary(0, VK.info.list.length - 1));
        }
        VK.info.actual = newPosition;

        if (document.querySelector(".info .label").innerHTML == VK.config.loadingSessionText) return;

        document.querySelector(".info .label").innerHTML = VK.info.list[VK.info.actual];
        $(".info .label").fadeIn(500);
        setTimeout(function () {
            $(".info .label").fadeOut(500);
            VK.info.change();
        }, VK.backgrounds.duration-500)
    }, 500)
