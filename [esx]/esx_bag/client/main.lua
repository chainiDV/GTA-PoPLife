...
function Bag()
    ...
    table.insert(elements, {label = 'Meter un objeto', value = 'put'})
    table.insert(elements, {label = 'Coger un objeto', value = 'take'})
    table.insert(elements, {label = 'Dejar caer bolsa', value = 'drop'})
...
AddEventHandler('esx-kr-bag:CheckBag', function()
    if HasBag then
        TriggerServerEvent('esx-kr-bag:putBag',true)
    else
        TriggerServerEvent('esx-kr-bag:putBag',false)
    end
end)
    ...
    while true do
        ...
        for i=1, #Bags, 1 do
            ...
            if GetDistanceBetweenCoords(playercoords, Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z, true) <= 1.5 and not HasBag then
                ...
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.45, '~g~[E]~w~ para coger la mochila')
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.35, '~o~[N]~w~ para buscar dentro de la mochila')
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys['5']) and HasBag and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsEntityInAir(PlayerPedId()) then -- Change F5 to the key you want to open the meny with
            Bag()
        end

        if IsEntityDead(GetPlayerPed(-1)) and HasBag == true then
            HasBag = false
            DropBag()
       end
    end
